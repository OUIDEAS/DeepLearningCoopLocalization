...
def test_convergence(num_anc):
    ...
    return converged, ann_mean, ann_dev, tri_mean, tri_dev, ax

def main():
    plt.figure()
    for i in range(7):
        print("Testing Convergence on ResNet with "+str(i+4)+" Anchors")
        n_anc = i+4
        converged, a,b,c,d, x = test_convergence(n_anc)
        print(converged)
        plt.plot(x, a)
        plt.plot(x, b)
        plt.plot(x, c)
