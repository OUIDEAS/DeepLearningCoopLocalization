    ...
    for i in range(n_generations):
        ...
        for e in range(epochs):
            ...
            for data, target, a1 in train_loader_no_noise:  # _no_noise:
                ...
                pos1, loss1 = GA1.train(data, target, a1)
                pos2, loss2 = GA2.train(data, target, a1)
                pos3, loss3 = GA3.train(data, target, a1)
                pos4, loss4 = GA4.train(data, target, a1)
                pos5, loss5 = GA5.train(data, target, a1)
                pos6, loss6 = GA6.train(data, target, a1)
                pos7, loss7 = GA7.train(data, target, a1)
                pos8, loss8 = GA8.train(data, target, a1)
                pos9, loss9 = GA9.train(data, target, a1)
                pos10, loss10 = GA10.train(data, target, a1)
                if a % 100 == 0:
                    e1.append(loss1)
                    e2.append(loss2)
                    e3.append(loss3)
...
def mutate(ranks, n_HL, n_HS, scale):
    ...
    n_HL[2] = hl1 + int(random.randrange(-1, 1, 1)/scale)
    n_HS[2] = hs1 + int(random.randrange(-50, 50, 1)/scale)
    n_HL[3] = hl1 + int(random.randrange(-1, 1, 1)/scale)
    n_HS[3] = hs1 + int(random.randrange(-50, 50, 1)/scale)
    n_HL[4] = hl1 + int(random.randrange(-1, 1, 1)/scale)
    n_HS[4] = hs1 + int(random.randrange(-50, 50, 1)/scale)
    n_HL[5] = hl1 + int(random.randrange(-1, 1, 1)/scale)
    n_HS[5] = hs1 + int(random.randrange(-50, 50, 1)/scale)
    n_HL[6] = hl2 + int(random.randrange(-1, 1, 1)/scale)
    n_HS[6] = hs2 + int(random.randrange(-50, 50, 1)/scale)
    n_HL[7] = hl2 + int(random.randrange(-1, 1, 1)/scale)
