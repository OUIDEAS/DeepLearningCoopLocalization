...
class Drone(object):
    ...
    def range(self, Drone1, Drone2, Drone3, Drone4):
...
def main():
    ...
    num_int = []
    aps = []
    for q in tqdm(range(len(dataset_lengths))):
        os.system('clear')
        dadjoke = Dadjoke()
        print(dadjoke.joke, '\n')
        if q > 0:
        ...
        with open(updates[q]+'_data.csv', 'w', encoding='UTF8') as f:
            ...
            for i in range(dataset_lengths[q]):
                ...
                for b in range(6):
                    avail.append([])
                    drone_list[0].range(drone_list[1],drone_list[2],drone_list[3],drone_list[4])
