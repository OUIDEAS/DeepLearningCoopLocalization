...
class Drone(object):
    ...
    def __init__(self, min_pos=0, max_pos=100):
        ...
        self.min_vel = -10
        self.dt = 0.01
        self.x_tru = rand.randrange(0, 100, 1) + rand.randrange(0, 100)/100
        self.y_tru = rand.randrange(0, 100, 1) + rand.randrange(0, 100)/100
...
def main():
    ...
    # for i in range(6):
    num_anchors = 7
    drone_list = [Drone(min_pos=0, max_pos=100)]

    for i in range(num_anchors):
        drone_list.append(Drone(min_pos=60, max_pos=100))


    dataset_lengths = [2000000, 10000]
    ...
    for q in range(len(dataset_lengths)):
        ...
        with open('Specialized_data/'+str(num_anchors)+updates[q]+'_data.csv', 'w', encoding='UTF8') as f:
            writer = csv.writer(f)
