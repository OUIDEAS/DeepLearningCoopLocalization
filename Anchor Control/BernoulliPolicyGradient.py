...
class BernoulliPolicy(nn.Module):
    def __init__(self):
        ...
        self.out = nn.ModuleList([DroneLayers(layers, size) for i in range(10)])
...
Transition = namedtuple('Transition',
                        ('state', 'action1', 'action2', 'action3', 'action4', 'action5', 'action6', 'action7', 'action8', 'action9', 'action10', 'next_state', 'reward'))
...
def main(wp, load):
    ...
    if load:
        ...
        rlav = []
        Actor = BernoulliPolicy().to(torch.device("cuda"))
    Actor_optim = torch.optim.Adam(Actor.parameters(), lr = actor_learning_rate, eps=1e-5)
    reward_list = []
    running_reward_loss = []
    env = environment.env()
    for i in range(num_episode):
        ...
        actions = []
        while not end:
            state, reward, end, new_state = env.Bstep(commands)
        ...
        for a in range(len(states)):
            ...
            pgloss = torch.sum(-probs[0].log_prob(actions[a][0].to(torch.device('cuda')))*rewards[a][0])
            pgloss.backward(retain_graph=True)
            pgloss =  torch.sum(-probs[1].log_prob(actions[a][1].to(torch.device('cuda')))*rewards[a][1])
            pgloss.backward(retain_graph=True)
            pgloss =  torch.sum(-probs[2].log_prob(actions[a][2].to(torch.device('cuda')))*rewards[a][2])
            pgloss.backward(retain_graph=True)
            pgloss =  torch.sum(-probs[3].log_prob(actions[a][3].to(torch.device('cuda')))*rewards[a][3])
            pgloss.backward(retain_graph=True)
            pgloss =  torch.sum(-probs[4].log_prob(actions[a][4].to(torch.device('cuda')))*rewards[a][4])
            pgloss.backward(retain_graph=True)
            pgloss =  torch.sum(-probs[5].log_prob(actions[a][5].to(torch.device('cuda')))*rewards[a][5])
            pgloss.backward(retain_graph=True)
            pgloss =  torch.sum(-probs[6].log_prob(actions[a][6].to(torch.device('cuda')))*rewards[a][6])
            pgloss.backward(retain_graph=True)
            pgloss =  torch.sum(-probs[7].log_prob(actions[a][7].to(torch.device('cuda')))*rewards[a][7])
            pgloss.backward(retain_graph=True)
            pgloss =  torch.sum(-probs[8].log_prob(actions[a][8].to(torch.device('cuda')))*rewards[a][8])
            pgloss.backward(retain_graph=True)
            pgloss =  torch.sum(-probs[9].log_prob(actions[a][9].to(torch.device('cuda')))*rewards[a][9])
            pgloss.backward(retain_graph = True)
