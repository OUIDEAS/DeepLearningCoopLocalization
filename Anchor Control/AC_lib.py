...
class SquashedGaussianMLPActor(nn.Module):
    ...
    def forward(self, obs, deterministic=False, with_logprob=True):
        net_out = self.net(obs.to(torch.float32))
...
class MLPQFunction(nn.Module):
    ...
    def forward(self, obs, act):
        q = self.q(torch.cat([obs.to(torch.float32), act.to(torch.float32)], dim=-1))
...
class MLPActorCritic(nn.Module):
    ...
    def __init__(self, observation_space, action_space, hidden_sizes=(1024,1024),
        ...
        self.pi = Policy(obs_dim, act_dim, hidden_size=1024)#SquashedGaussianMLPActor(obs_dim, act_dim, hidden_sizes, activation, act_limit)
