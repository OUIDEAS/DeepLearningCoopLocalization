...
class SAC():
    def __init__(self, n_in, hidden_dim, n_actions):
        ...
        self.pi_optim = Adam(self.ac.pi.parameters(), lr = 1e-7)
    ...
    def calc_q_loss(self, o, a, r, o2):
        ...
        with torch.no_grad():
            ...
            a2, logp_a2, _ = self.ac.pi.sample(o2)
    ...
    def compute_loss_pi(self, o):
        pi, logp_pi, _ = self.ac.pi.sample(o)
    ...
    def select_actions(self, state, deterministic=False):
        return self.ac.choose_actions(torch.as_tensor(state, dtype=torch.float32), deterministic)

class OptimSolver():
    def __init__(self, method: str = "Powell", num_actions:int = 30, tolerance:int = 1):
        self.method = method
    ...
    def reward(self, actions, env):
        _, reward, _, _ = self.env.step(actions)
        score = 1/reward
        # new_state = self.env.avail
        # try:
        #     score = OLSsolver.PDOP_Solver(new_state)
        # except:
        #     score = 1000
    ...
    def get_actions(self, env):
        actions = scipy.optimize.minimize(self.reward, [0 for i in range(self.num_actions)], method=self.method, args=(env), tol = self.tol)
        return actions.x     
...
class Runner():
    def __init__(self, number_of_episodes, number_of_anchors = 3, device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')):
        ...
        self.AnchorControl = OptimSolver("Nelder-Mead", num_actions=number_of_anchors*3, tolerance = 1)
    ...
    def run_episode(self):
        ...
        self.AnchorControl.set_current_state(state)
        self.AnchorControl.copy_env(self.env)
        self.state = []
        self.action = []
        self.reward = []
        self.new_state = []
        ...
        while not end:
            self.AnchorControl.copy_env(self.env)
            action = self.AnchorControl.get_actions(self.env)
...
if __name__ == "__main__":
    ...
    print("Training SAC network to account for ", 5, " anchors.")
    agent = Runner(number_of_episodes = n_episodes, number_of_anchors = 5)
    agent.train()
    plt.figure()
    ...
    plt.title(str(5)+' Anchor Drones: Reward vs Episode')
    plt.legend()
